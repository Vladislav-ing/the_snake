...
class Snake(GameObject):
    ...
    def __init__(self, body_color=SNAKE_COLOR, length=1, direction=RIGHT, next_direction=None):
    ...
    def move(self):
        ...
        update_head = [
                   head[0] + (direct[0] * GRID_SIZE), 
                   head[1] + (direct[1] * GRID_SIZE)
    ]
     
        if (0 >= update_head[0] or update_head[0] >= SCREEN_WIDTH) or (0 >= update_head[1] or update_head[1] >= SCREEN_HEIGHT):
            update_head = [
                           update_head[0] % SCREEN_WIDTH,
    ...
    def reset(self):
        ...
        self.__init__(SNAKE_COLOR, 1,direction=new_direct, next_direction=None)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key in dict_input:
                ...
                game_object.next_direction = next_path if abs(actuall_path[0]) != abs(next_path[0]) else None
